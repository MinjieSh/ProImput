...
CAMASest_impu <-
    function(MGResult,
        ...
        if (is(data, "data.frame")) {
            ...
            stop(
                "Only matrix, data frame, SummarizedExperiment and ExpressionSet
        ...
        Xprep <- PrepResult@Xprep
        Xclu <- PrepResult@Xproj
        dataSize <- ncol(Xprep)
        
        Aproj <-
            PrepResult@centers[, as.character(MGResult@corner[corner.strategy,])]
        Kest <- ncol(Aproj)
        ...
        if (generalNMF == FALSE) {
            ...
            scale <- tryCatch({
                c(NMF::.fcnnls(Aproj, PrepResult@W %*% PrepResult@SW[1:dim(Xprep)[1]])$coef)
            }, error = function(msg) {
                res <-
                    apply(PrepResult@W %*% PrepResult@SW[1:dim(Xprep)[1]], 2, function(x)
                        (nnls::nnls(Aproj, x))$x)
            ...
            scale[scale < 1e-10] <-
                0.01 / (sqrt(colSums(Aproj ^ 2)))[scale < 1e-10]
        }
        ...
        S1 <- tryCatch({
            ...
            return(apply(Xprep, 2, function(x)
                (nnls::nnls(Apca, x))$x))
        })
        ...
        if (generalNMF == FALSE) {
            scale.org <- tryCatch({
                c(NMF::.fcnnls(Aproj.org, matrix(PrepResult@SW[1:dim(Xprep)[1]], ncol =
                                                     1))$coef)
            }, error = function(msg) {
                res <-
                    apply(matrix(PrepResult@SW[1:dim(Xprep)[1]], ncol = 1), 2, function(x)
                        (nnls::nnls(Aproj.org, x))$x)
                return(as.vector(res))
            })
        ...
        S2 <- tryCatch({
            NMF::.fcnnls(Aest.org, Xclu)$coef
        }, error = function(msg) {
            return(apply(Xclu, 2, function(x)
                (nnls::nnls(Aest.org, x))$x))
        })
        datalength2 <- (nrow(Xclu) * dataSize) / 2 *
            log(mean((as.vector(
                Xclu - Aest.org %*% S2
            )) ^ 2))
        reconErr2 <- sum((as.vector(Xclu - Aest.org %*% S2)) ^ 2)
        penalty2 <- ((Kest - 1) * nrow(Xclu)) / 2 * log(dataSize) +
            (Kest * dataSize) / 2 * log(nrow(Xclu))
        ...
        noNaNnls <- function(x) {
            ind <- !is.na(x)
            nnls::nnls(t(S2[, ind]), x[ind])$x
        }
        Aest.full <- t(apply(data, 2, noNaNnls))
        
        
        MGlist <-
            lapply(as.character(MGResult@corner[corner.strategy,]),
                   function(x)
                       colnames(PrepResult@Xproj)[PrepResult@cluster$cluster == x])
        if (appro3) {
            ...
            if (generalNMF == FALSE) {
                ...
                scale.all <- tryCatch({
                    ...
                    res <-
                        apply(matrix(PrepResult@SW, ncol = 1), 2, function(x)
                            (nnls::nnls(Aproj.all, x))$x)
                    return(as.vector(res))
                })
