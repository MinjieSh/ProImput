...
CAMPrep3 <-
    function(data,
        ...
        if (generalNMF == FALSE) {
            ...
            C1 <- P1 %*% Xmeanrm
            Xmeanrm <-
                Xmeanrm - matrix(rep(C1, 1, each = Lorg), Lorg) *
        ...
        if (lof.thres > 0) {
            ...
            lof.outlier <-
                lof.factor > quantile(lof.factor, 1 - lof.thres)
        ...
        if (!is.null(quick.select) &&
            quick.select < length(corner)) {
