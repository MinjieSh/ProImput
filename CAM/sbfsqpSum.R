sbfsqpSum <- function(Xall, Aall) {
    minK <- 1
    
    costfunc <- function(combset) {
        A <- Aall[, combset, drop = FALSE]
        fit.err <- qpSum(A, Xall)
    }
    
    rmworst <- function(combset) {
        fit.err <- unlist(lapply(seq_along(combset), function(x)
            costfunc(combset[-x])))
    ...
    while (length(currset) > minK) {
        ...
        fit.err <-
            unlist(lapply(outset, function(x)
                costfunc(c(currset, x))))
        addelem <- outset[which.min(fit.err)]
        
        if (remelem != addelem) {
            while (min(fit.err) < sbfscost[length(currset) + 1]) {
