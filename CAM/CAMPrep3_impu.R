...
CAMPrep3_impu <-
  function(data,
    ...
    #show(dim(data))
    #show(which(rowSums(data, na.rm=TRUE) <= 0))
    ...
    Xorg <- X
    L <- min(dim.rdc, Lorg)
    Valid <- rep(TRUE, dataSize)
    ################ sample clustering #################
    if (length(cluster.method) > 1)
      cluster.method <- cluster.method[1]
    if (cluster.method == 'K-Means') {
      cosSim <- matrix(NA, Lorg, Lorg)
      for (i in seq_len(Lorg)) {
        for (j in seq_len(Lorg)) {
          rmInd <- unique(c(which(is.na(X[i,])), which(is.na(X[j,]))))
          if (length(rmInd) != 0) {
            cosSim[i, j] <- lsa::cosine(X[i, -rmInd], X[j, -rmInd])
          } else {
            cosSim[i, j] <- lsa::cosine(X[i, ], X[j, ])
          }
        }
      }
      newDis <- 1 / cosSim
      ...
      samCluRes <- cluster::pam(newDis, L, diss = TRUE)
      ## repeat K-medoids and use the best one
      #if (ncol(Xproj) > 15000) {
      #    ntry <- 10
      #} else {
      ntry <- 50
      ...
      for (i in seq_len(ntry)) {
        tmp <-
          cluster::pam(newDis,
                       L,
                       medoids = sample(Lorg, dim.rdc),
                       diss = TRUE)
        if (samCluRes$objective[2] > tmp$objective[2]) {
          samCluRes <- tmp
        }
      }
    ...
    Xclu <- matrix(NA, L, dim(Xorg)[2])
    for (n in seq_len(L)) {
      Xclu[n, ] <-
        colMeans(Xorg[samCluRes$clustering == n, , drop = FALSE], na.rm = TRUE)
    }
    Xclu[is.na(Xclu)] <- 0
    ...
    if (generalNMF == FALSE) {
      ...
      P1 <- sample.weight[1:L]
      P1 <- P1 / sqrt(sum(P1 ^ 2))
      C1 <- P1 %*% Xmeanrm
      Xmeanrm <- Xmeanrm - matrix(rep(C1, 1, each = L), L) *
        matrix(P1, L, dataSize)
      r <- svd(Xmeanrm)
      Ppca <- t(r$u[, seq_len(L - 1)])
      X <- rbind(P1, Ppca) %*% Xclu
      weightMatrix <- rbind(P1, Ppca)
    } else {
      #standard PCA
      r <- svd(Xmeanrm)
      Ppca <- t(r$u[, seq_len(L)])
      X <- Ppca %*% Xclu
      if (sum(X[1,] > 0) > 0 &&
          sum(X[1,] < 0) > 0 || sum(X[1,] == 0) > 0) {
