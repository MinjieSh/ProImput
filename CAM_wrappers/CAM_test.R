...
CAM_NIPALS_test <- function() {
  ...
  err <- c()
  for (i in 2:10) {
    err <- c(err, nrmse_full_data(grnd_truth, misg_data, res$res[[i]]))
  }
  plot(err)
  
  print(nrmse_full_data(grnd_truth, misg_data, res$init_data))
}
...
CAM_cmplt_test <- function(grnd_truth) {
  misg_data <-
    full_data_mask_out(grnd_truth, option = "MAR", 0.3, keepOption = "max")$data_extra_MV
  ...
  predicted <- impute(misg_data,
                      "mean")$res
  print(nrmse_full_data(grnd_truth, misg_data, predicted))
}
